...
trait ConsoleTools
{
     ...
     * @return void
     */
    protected function publishesMigrationsCustom(string $package, bool $isModule = false): void
    {
        ...
        if (file_exists($path = $basePath.'/database/migrations')) {
            ...
            $migrations = collect($stubs)->flatMap(function ($migration) use ($existing, $package) {
                ...
                return [$migration => $this->app->databasePath('migrations/'.$package.'/'.($match ? basename($match) : date('Y_m_d_His', time() + (int)mb_substr($sequence, -6)).str_replace($sequence, '', basename($migration))))];
            })->toArray();
